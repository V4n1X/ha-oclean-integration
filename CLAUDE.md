...
**Firmware fragility (field-confirmed 2026-09-09).** A live test against a real OCLEANY3S (fw 1.0.0.19) froze the brush (software *and* hardware buttons unresponsive) **twice**, both times during a run that *wrote* commands. Read-only sessions (connect, GATT dump, DIS, battery) always ran fine. Keep the BLE traffic byte-identical to the APK: do **not** send `0202` (`clearRunningDate`), do **not** pre-clear the CCCD, do **not** pair, and route commands exactly as above. See `docs/OCLEANY3S-AUDIT.md` §2.9–2.15.

**No CCCD on the notify characteristics (hardware-confirmed).** OCLEANY3S fw 1.0.0.19 exposes a Client Characteristic Configuration descriptor only on `…bb91` (unused by the app) and `0x2A19` — **not** on `…bb86`, `…bb88` or `…bb90`. Android's `setCharacteristicNotification()` needs none, so the official app works; WinRT cannot subscribe at all (the CCCD write is answered with ATT `0x03` "Write Not Permitted"), BlueZ tolerates it. `_has_cccd()` detects this and takes the no-CCCD path directly instead of issuing a doomed descriptor write.

> **Do not poll this device from Windows-native BLE.** Without a CCCD the responses cannot be received there, so a poll cannot succeed — and writing commands anyway has hung the hardware. Validate on the real HA host (Linux/BlueZ) or through an ESPHome proxy.
...
- A zone is "covered" (diagram level 3) when `norm[k] >= threshold`. 8-zone path thresholds map by the `cc.a`/`EnumC1322a` device enum (confirmed: each model's `protocol` int → enum constant): **OCLEANY3PD → 10**; OCLEANY3M / OCLEANY3 / OCLEANY3P / OCLEANY3S → **9** (default); YD0003 → 8. 12-zone path uses `<=2.0 → 1, <5.5 → 2, else 3`. There is **no literal "7 s" threshold anywhere** in the APK — the old v1.3.1 note was wrong.
- Our parsers implement this exactly: `share_threshold = AREA_COVERAGE_NORM_THRESHOLD(=9) / duration`, then a zone counts when `raw/sum >= share_threshold` (see `_build_area_stats` and `const.py`). The coordinator passes `AREA_COVERAGE_Y3PD_THRESHOLD(=10)` to the record parser when the DIS model is exactly `OCLEANY3PD`. So coverage scales with session length and matches the per-model threshold.
...
**Other record fields analysed from the APK (2026-06-16):**
- **`pressureCode` — IMPLEMENTED** (`_pressure_code`, sensor `last_brush_pressure_code` / "Pressure Level"). Faithfully replicates APK `a.b.m14b` (smali `a/b.smali`) over the 5 `pressureRatio` values (bytes 11-15 — the APK's `iBytesToIntBe10..14` are the 10th-14th *reads*, i.e. those same bytes, NOT byte offsets 10-14). It returns the *dominant pressure bucket* as `0/50/60/70/80/90`: a heavier dominant bucket → higher code (index 4 dominant → 90); `values[0] >= 85` → 0 (predominantly light). Mapping: single dominant idx 0/1→60, 2→70, 3→80, 4→90; two tied buckets keyed by index-sum (1-2→50, 3-4→60, 5-6→70, 7→80); 3 tied→80; 4-5 tied→90. The exact user-facing label is not in the APK; documented as a diagnostic.
- **`gestureCode` (2-bit, byte 30)** — stored as the DB `gesture` column / `BrushRecordEntity.gesture`. A second, exhaustive search (sources + smali + `resources/res/values*` strings) found **no 0-3 → label mapping anywhere**: the value is extracted, stored and uploaded but never displayed or switched on. Confirmed unresolvable from this APK. Kept as the raw `last_brush_gesture_code` field; not surfaced as a labelled sensor.
- **`powerArray` (12× 2-bit, bytes 30-32)** — stored as `BrushRecordEntity.powerArray`. No code maps the recorded 0-3 nibbles to labels. **Lead (unconfirmed):** the only 4-level power scale in the app is the plan-customisation strings `plan_set_power_extremely_low / low / medium / higher` ("Very Low/Low/Medium/High"), which matches a 0-3 range — but these are used for *setting* brush power, not wired to the recorded `powerArray`. So `powerArray[k]` is *plausibly* a per-zone power-intensity level (0=Very Low … 3=High), but this is not code-confirmed. Kept as the raw `last_brush_power_array` field; not surfaced as labelled sensors until confirmed.
- **Extended command set (`0241-0245`, `02A1-02A5`, `0313`)** — only used by APK handlers `C3367n0`/`C3387x0` for generic models `0008/0009/000E/000F`, which we do not support. No action.
